...
watch(() => props.entryId, () => {
	data.doAsync(async () => fetchJSONOrThrow(`/api/v1/entry/${props.entryId}/`));
}, { immediate: true });
...
async function submitImage(value) {
	data.doAsync(async () => {
		...
		const response = await postOrThrow(`/api/v1/entry/${props.entryId}/image`, formData);
...
async function submitText(unsafeText: string) {
	const response = await postOrThrow(
		`/api/v1/entry/${props.entryId}/text`,
	...
	<div class="tins-entry">
		<tins-status-helper :error="data.error.value" :loading="data.loading.value">
			<template v-if="!data.loading.value">
				...
				<p>
					<a :href="`/${competition.short}/reviews/entry/${props.entryId}/`" router-ignore>Reviews ({{entry.reviewCount}})</a>
				</p>
			</template>
		</tins-status-helper>
	</div>
...
<style scoped>
	.tins-entry :deep(a) { font-weight: bold; text-decoration: none; }
	.tins-entry :deep(a:link) { color: #600; }
	.tins-entry :deep(a:hover) { text-decoration: underline; }
	.tins-entry :deep(a:active) { text-decoration: underline; }
